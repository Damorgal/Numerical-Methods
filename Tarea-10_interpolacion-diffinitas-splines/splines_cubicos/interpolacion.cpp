// Funciones hechas para la tarea 9 y 10 de interpolacion
...
double* minimiza_diferencias(double *px, double *py, double *x, double lm, double h, int n, int m)	{
	...
	inicializa_vec(d2,0,n+1);
	inicializa_vec(d3,0,n+1);
	
	// Creamos sistema de ecuaciones
	double lh = lm/h;
	for(int k=0; k<n; k++)	{
		for(int i=0; i<m; i++)	{
			d2[k] += pow(Nk(x,px[i],h,k,n), 2);
			d3[k] += Nk(x,px[i],h,k,n) * Nk(x,px[i],h,k+1,n); 
			d2[k+1] += pow(Nk(x,px[i],h,k+1,n), 2);
		}
		d2[k] += lh;
		d2[k+1] += lh;
		d3[k] -= lh;
	...
	// Creamos las ordenadas
	for(int k=0; k<n; k++)
		for(int i=0; i<m; i++)	{
			ordenadas[k] += py[i]*Nk(x,px[i],h,k,n);
			ordenadas[k+1] += py[i]*Nk(x,px[i],h,k+1,n);
		}
	
	// Resolvemos el sistema tridiagonal
	double *fis = resuelve_tridiagonal(d1,d2,d3,ordenadas,n+1);
